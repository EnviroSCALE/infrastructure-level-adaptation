...
def reconstruct(packed):
	...
	for i in range(N):
		...
		#timestring = time
		listq.append(Reading(id, value, time))
		print (c["event"][id]["name"], value, timestring)
	sortedlist = sorted(listq)

	for reading in sortedlist:
		print (reading)
		delt = (reading.time - starttimes[0])
		val = reading.value
		id = reading.event_id
		with open("recon_"+c["sensor"][id]["trace"], 'a') as the_file:
			the_file.write(str(delt)+'\t'+str(val)+ '\n')
				
			#~ with open('dump.bin', 'a') as newFile:
...
def upload_a_bundle(readings_queue):
	try:
		...
		CircuitsApp.total_uploaded_calculated += len(packed) + c["params"]["beta"]
		reconstruct(packed)
		return
...
class Reading(object):
	...
	def __eq__(self, other):
		return self.time == other.time and self.value == other.value  and self.event_id == other.event_id
...
class ReadHandler(Component):
	def read_and_queue(self, sensor, readings_queue):
		...
		if c["sensor"][sensor.id]["type"] == "virtual":
			if random.uniform(0, 1) < (1.0/sensor.period)/sensor.F:
				return
	...
	def read_event(self, *args, **kwargs): 
		...
		if c["sensor"][s1.id]["type"] == "virtual":
				CircuitsApp.timers["sense"] = Timer(1.0/s1.F, ReadEvent(s1, q), persist=False).register(self)
		else:
...
class UploadHandler(Component):      
	...
	def upload_event(self, *args, **kwargs):
		...
		CircuitsApp.last_uploaded = CircuitsApp.total_uploaded_calculated
		...
		CircuitsApp.timers["period_update"] = Timer(0.01, PeriodUpdateEvent(), persist=False).register(self)
...
class PeriodUpdateHandler(Component):
	...
	def update(self, *args, **kwargs):        
		...
		if CircuitsApp.choice == 1:
			A = CircuitsApp.aComputer.computeA(CircuitsApp.last_uploaded, t)
			for i in range(0, k):
				CircuitsApp.sensors[i].period = 1.0 * k * CircuitsApp.sensors[i].b / (A * 5.0)
				
		if CircuitsApp.choice == 3:
			A = CircuitsApp.aComputer.computeA(CircuitsApp.last_uploaded, t)
			for i in range(0, k):
				CircuitsApp.sensors[i].period = 1.0 * CircuitsApp.sensors[i].b / (A * 5.0 * CircuitsApp.sensors[i].weight)
...
class EndHandler(Component):
	...
	def end_event(self, *args, **kwargs):
		...
		result = [c["choice"], c["params"]["D"],c["interval"]["M"],c["interval"]["upload"], c["interval"]["period_update"], util, goodput ]
		if CircuitsApp.shareState == 0:
			lprint(EventReport("EndEvent", "UPLOAD"))
			#yield self.fire(task(UploadEvent(CircuitsApp.readings_queue)))
		lprint(EventReport("Utilization", str( CircuitsApp.total_uploaded_calculated)))#*100.0/c["params"]["D"]   )  ) )
...
class App(Component):
	...
	def init_scene(self):       
		...
		c["interval"]["M"] = int(MM)
		c["params"]["D"] = int(DD)
		c["interval"]["period_update"] = int(PP)
